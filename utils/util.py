...
def load_folds_data(np_data_path, n_folds):
    ...
    files_pairs = [files_dict[key] for key in sorted(files_dict.keys())]
    files_pairs = np.array(files_pairs, dtype=object)
    # Handle small/incomplete datasets: use only permutation indices within range
    if len(files_pairs) > 0:
        valid_idx = r_permute[r_permute < len(files_pairs)]
        if len(valid_idx) > 0:
            files_pairs = files_pairs[valid_idx]

    # Ensure n_folds does not exceed number of subject pairs
    n_subjects = len(files_pairs)
    if n_folds > n_subjects and n_subjects > 0:
        n_folds = n_subjects
