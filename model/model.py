...
class MRCNN(nn.Module):
    def __init__(self, afr_reduced_cnn_size):
        ...
        # NEW: force temporal dimension to 80 (d_model) so TCE LayerNorm(size=80) matches last dim
        self.target_len = 80
        self.adapt_pool = nn.AdaptiveAvgPool1d(self.target_len)
    ...
    def forward(self, x):
        x1 = self.features1(x)
        x2 = self.features2(x)
        # align temporal dims of both branches using adaptive pooling, then concat along time
        t_common = min(x1.size(2), x2.size(2))
        if x1.size(2) != t_common:
            x1 = F.adaptive_avg_pool1d(x1, t_common)
        if x2.size(2) != t_common:
            x2 = F.adaptive_avg_pool1d(x2, t_common)
        x_concat = torch.cat((x1, x2), dim=2)
        # force final temporal length to target_len expected by attention/FC
        if x_concat.size(2) != self.target_len:
            x_concat = self.adapt_pool(x_concat)
